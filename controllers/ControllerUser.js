...
ControllerUser.get("/", [MiddlewareAuth.requiredToken], async (req, res) => {
  try {
    const result = await DAOUser.getByEmail(req.query.email);
...
ControllerUser.put(
  "/:email",
  [MiddlewareAuth.requiredToken],
  async (req, res) => {
    try {
      const result = await DAOUser.update(
        req.params.email,
        req.body.is_superuser,
        req.body.is_staff,
        req.body.status
